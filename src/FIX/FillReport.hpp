...
namespace fix
{
  struct FillReport
  {
    ...
    FillReport() = default;
    FillReport(LOB::UID aggressor, LOB::UID resting, LOB::Quantity qty, LOB::Price price,
               LOB::Side s)
        : aggressor_uid_(aggressor), resting_uid_(resting), filled_qty_(qty), exec_price_(price),
          side_(s)
    {
    }
  };
