...
SCENARIO("Adding single order to limit tree")
{
    ...
    GIVEN("A limit tree and a single buy order")
    {
        ...
        WHEN("Order is added")
        {
            auto o = std::make_shared<Order>(1, price, Side::BUY, qty);
            lt.limit(o);
            THEN("The tree adjusts to record the new order added")
            {
                ...
                REQUIRE(o == lt.best->orderList.back());
                REQUIRE(o == lt.best->orderList.front());
                REQUIRE(lt.best == o->parentLimit);
            }
        }
    }
    GIVEN("A limit tree and a single sell order")
    {
        ...
        WHEN("Order is added")
        {
            auto o = std::make_shared<Order>(1, price, Side::SELL,qty);
            lt.limit(o);
            THEN("The tree adjusts to record the new order added")
            {
                REQUIRE(lt.limits.size() == 1);
