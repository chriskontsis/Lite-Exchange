...
struct OrderRequest
{
  ...
  LOB::SymbolId    symbol_id{0};
  LOB::TimeInForce time_in_force{LOB::TimeInForce::GTC};
};

class FixMessage
{
 public:
  static OrderRequest parse(std::string_view rawOrder, gateway::SymbolRegistry& symbols)
  {
    OrderRequest req;
      ...
      if (eq != std::string_view::npos)
      {
        ...
        switch (tag)
        {
          ...
          case 44:
            ...
            break;
          case 55:
            req.symbol_id = symbols.getOrCreate(val);
            break;
          case 59:
            if (!val.empty() && val[0] == '3')
              req.time_in_force = LOB::TimeInForce::IOC;
            break;
        }
      }
